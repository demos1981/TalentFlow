-- Скрипт для повного перестворення бази даних TalentFlow з camelCase
-- Запустіть через Railway CLI: railway connect postgres < recreate-database-camelcase.sql
...
-- 4. Створюємо таблицю users з camelCase назвами колонок
CREATE TABLE users (
    id SERIAL PRIMARY KEY,
...
-- 5. Створюємо таблицю jobs з camelCase назвами колонок
CREATE TABLE jobs (
    id SERIAL PRIMARY KEY,
...
CREATE TABLE interviews (
    ...
    type VARCHAR(50), -- phone, video, onsite
...
SELECT 'Database recreated successfully with camelCase column names!' as message;
