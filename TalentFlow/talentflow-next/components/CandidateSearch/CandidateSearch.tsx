...
import { candidateService, CandidateFilters, CandidateSearchResponse, Candidate, CandidateSearchParams } from '../../services/candidateService';
...
const CandidateSearch: React.FC = () => {
  ...
  const [currentFilters, setCurrentFilters] = useState<CandidateSearchParams>({
    page: 1,
    limit: 20,
    sortBy: 'relevance',
    sortOrder: 'DESC'
  ...
  // Перевіряємо чи користувач має право шукати кандидатів
    const canSearchCandidates = user?.role === USER_TYPES.EMPLOYER || user?.role === USER_TYPES.ADMIN;
  ...
  const performSearch = async (filters: CandidateSearchParams) => {
    if (!canSearchCandidates) {
  ...
  const handleFiltersChange = (newFilters: Partial<CandidateSearchParams>) => {
  ...
  const handleResetFilters = () => {
    const defaultFilters: CandidateSearchParams = {
      page: 1,
      limit: 20,
      sortBy: 'relevance',
      sortOrder: 'DESC'
