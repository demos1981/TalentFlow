...
export const ClientJobDetailContent: React.FC<ClientJobDetailContentProps> = ({ job, error }) => {
  ...
  const { isSaved, toggleJob } = useSavedJobsStore();
  ...
  // Обробка збереження/видалення вакансії
  const handleSave = () => {
    if (job) {
      toggleJob(job.id);
    }
      ...
      <div className="job-detail-header">
        ...
        <div className="job-actions">
          <button 
            ...
            onClick={handleSave}
            style={{
              backgroundColor: job && isSaved(job.id) ? '#7c3aed' : '',
              color: job && isSaved(job.id) ? '#fff' : ''
            }}
          >
            <Star className={`icon ${job && isSaved(job.id) ? 'fill' : ''}`} />
            {job && isSaved(job.id) ? t('saved') : t('save')}
          </button>
