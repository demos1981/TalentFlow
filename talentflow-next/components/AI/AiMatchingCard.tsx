...
import { RecommendationType, MATCH_SCORE_LABELS, MATCH_SCORE_COLORS } from '../../constants/aiMatching';
...
const AiMatchingCard: React.FC<AiMatchingCardProps> = ({
  ...
  const getMatchScoreColor = (score: number) => {
    if (score >= 90) return MATCH_SCORE_COLORS[90];
    if (score >= 80) return MATCH_SCORE_COLORS[80];
    if (score >= 70) return MATCH_SCORE_COLORS[70];
    return MATCH_SCORE_COLORS[60];
  };

  const getMatchScoreLabel = (score: number) => {
    if (score >= 90) return MATCH_SCORE_LABELS[90];
    if (score >= 80) return MATCH_SCORE_LABELS[80];
    if (score >= 70) return MATCH_SCORE_LABELS[70];
    return MATCH_SCORE_LABELS[60];
  };
        ...
        <div className="ai-matching-card-avatar">
          {recommendation.avatar ? (
            ...
            <div className="ai-matching-card-avatar-placeholder">
              {recommendation.type === RecommendationType.CANDIDATE ? <User size={24} /> : <Briefcase size={24} />}
            </div>
          )}
        </div>
